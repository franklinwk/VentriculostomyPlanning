...
class VentriculostomyPlanning(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.contributors = ["Junichi Tokuda (BWH)", "Longquan Chen (BWH)", "Franklin King (BWH)"] # replace with "Firstname Lastname (Organization)"
    self.parent.helpText = """This is an scripted module for ventriclostomy planning"""
    self.parent.acknowledgementText = """This module was developed based on an example code provided by Jean-Christophe Fillion-Robin, Kitware Inc.
    and Steve Pieper, Isomics, Inc. and was partially funded by NIH grant 3P41RR013218-12S1.:""" # replace with organization, grant and thanks.
...
class VentriculostomyPlanningWidget(ScriptedLoadableModuleWidget, ModuleWidgetMixin):
  ...
  def setup(self):
    ...
    self.venousMarginEdit = self.createLineEdit(title="", text='5.0', readOnly=False, frame=True, maxLength = 6,
    ...
    self.startVesselSegmentationButton = self.createButton(title="",
                                                    toolTip="Start venous segmentation",
                                                    enabled=False, checkable = True,
                                                    maximumHeight=self.buttonHeight, maximumWidth=self.buttonWidth,
                                                    icon=self.createIcon("vessel.png", self.scriptDirectory),
                                                    iconSize=qt.QSize(self.buttonHeight, self.buttonWidth))
    self.startVesselSegmentationButton.connect('clicked(bool)', self.onPerformVesselSegmentation)
    self.mainGUIGroupBoxLayout.addWidget(self.startVesselSegmentationButton, 2, 3)
  ...
  def updateFromCaseManager(self, EventID):
    if EventID == self.slicerCaseWidget.CloseCaseEvent:
      ...
      if self.logic.baseVolumeNode and self.logic.ventricleVolume:
        ...
        self.SerialAssignBox.volumesCheckedDict = { "Venous" : self.logic.baseVolumeNode, "Ventricle": self.logic.ventricleVolume,}
      ...
      self.startVesselSegmentationButton.setEnabled(False)
  ...
  def updateFromLogic(self, EventID):
    ...
    if EventID == VentriculostomyUserEvents.VentricleCylinderModified:
      ...
      ventriclePointLeft = -numpy.sign(numpy.dot(distalPos - targetPos, normalVec))  # here left hemisphere means from the patient's perspective
  ...
  def onSelect(self, selectedNode=None):
    ...
    if selectedNode:
      ...
      if self.logic.ventricleVolume and self.logic.baseVolumeNode:
        ...
        self.logic.enableRelatedModel("vtkMRMLScalarVolumeNode.rel_cylinderModel", "cylinderModel", color=[0.0, 1.0, 1.0])
        ...
        self.progressBar.labelText = 'Segmenting Skin Surface'
        slicer.app.processEvents()
        self.createModel()

        self.progressBar.value = 25
        self.progressBar.labelText = 'Identifying Vessels and Creating Path'
        slicer.app.processEvents()
        self.onConnectedComponentCalc()
        self.prepareCandidatePath()
        self.progressBar.value = 75
        self.logic.calculateCylinderTransform()
        self.setBackgroundAndForegroundIDs(foregroundVolumeID=self.logic.ventricleVolume.GetID(), backgroundVolumeID=self.logic.baseVolumeNode.GetID())
        self.onSet3DViewer()
        self.progressBar.labelText = 'Saving Preprocessed Data'
        slicer.app.processEvents()
        self.onSaveData()
        self.progressBar.close()
        self.logic.appendPlanningTimeStampToJson(self.jsonFile, "EndPreprocessing", datetime.datetime.now().time().isoformat())
    ...
    if self.logic.baseVolumeNode:
      ...
      if targetNodeID and distalNodeID:
        ...
        if targetNode.GetNumberOfFiducials() and distalNode.GetNumberOfFiducials():
          self.startVesselSegmentationButton.setEnabled(True)
      ...
      if targetNodeID and distalNodeID and clippedVolumeNodeID and croppedVolumeNodeID and clipModelNodeID:
          ...
          if targetNode.GetNumberOfFiducials() and distalNode.GetNumberOfFiducials() and clipModelNode:
            ...
            if self.volumePrepared == False:
              ...
              # -----------
              ventricleDirect = (posDistal - posTarget) / numpy.linalg.norm(posTarget - posDistal)
              ...
              angle = 180.0 / math.pi * math.atan(2 * self.logic.cylinderRadius / numpy.linalg.norm(posTarget - posDistal))
              ROIRadiusXYZ = [150, coneHeight * ventricleDirect[1], max(coneHeight * math.tan(angle * math.pi / 180.0), coneHeight * ventricleDirect[2])]
            ...
            self.setBackgroundAndForegroundIDs(foregroundVolumeID=self.logic.ventricleVolume.GetID(), backgroundVolumeID=quarterVolume.GetID(), fitToSlice=True)
  ...
  # Try out candidate paths
    ...
    if targetNodeID and distalNodeID and nasionNodeID:
      ...
      if targetNode.GetNumberOfFiducials() and distalNode.GetNumberOfFiducials() and nasionNode.GetNumberOfFiducials():
        ...
        # I believe new versions of VTK add 0,0,0 as an intersection point causing a bug
        if numpy.array_equal(posEntry, [0,0,0]):
          FiducialPointAlongVentricle.GetNthFiducialPosition(1,posEntry)
        ...
        phiResolution = 1*numpy.pi/180.0 # Increase number to reduce candidates generated
        #phiResolution = 100*numpy.pi/180.0
        radiusResolution = 1.0
        #radiusResolution = 12.0
        ...
        if self.logic.synthesizedData.GetNumberOfPoints() and numOfRef >= 1:
          ...
          if self.logic.nPathReceived <= 0:
            slicer.util.warningDisplay("No cannula candidate is venous-collision free, consider redefining the ventricle area.")
            self.disableVentricleModification(False)
            return False
          else:
            status = self.logic.makePathMeetAllConditions(self.logic.pathReceived, self.logic.nPathReceived, self.logic.pathCandidatesModel.GetPolyData(), posKocher, posNasion, surfacePolyData)
            ...
            if status == CandidatePathStatus.NoPosteriorAndNoWithinKocherPoint:
              slicer.util.warningDisplay("Cannula candidates are both too close to the forehead and too far from the kocher's point, consider redefining the ventricle area.")
            elif status == CandidatePathStatus.NoPosteriorPoint:
              slicer.util.warningDisplay("Cannula candidates are too close to the forehead, consider redefining the ventricle area.")
            elif status == CandidatePathStatus.NoWithinKocherPoint:
              slicer.util.warningDisplay("Cannula candidates are too far from the kocher's point, consider redefining the ventricle area.")
            else:
  ...
  def onCreatePlanningLine(self):
    ...
    if not self.logic.baseVolumeNode:
      ...
      if self.logic.createPlanningLine():
        ...
        guideTransformWithTranslate.PostMultiply()
        guideTransformWithTranslate.Translate(entryPos)
        guideTransformFilter = vtk.vtkTransformPolyDataFilter()
        guideTransformFilter.SetInputData(guidePlatform)
        guideTransformFilter.SetTransform(guideTransformWithTranslate)
        guideTransformFilter.ReleaseDataFlagOn()
        guideTransformFilter.Update()
        guidePlatform = guideTransformFilter.GetOutput()

        # Cut away area under skin from guide platform and mask
        skullModelNodeID = self.logic.baseVolumeNode.GetAttribute("vtkMRMLScalarVolumeNode.rel_model")
        skullModel = None
        if skullModelNodeID:
          skullModelNode = slicer.mrmlScene.GetNodeByID(skullModelNodeID)
          skullModel = skullModelNode.GetPolyData()
        skullTransform = vtk.vtkTransform()
        skullTransform.Translate(insertionVector*(-2.0))
        skullTransformFilter = vtk.vtkTransformPolyDataFilter()
        skullTransformFilter.SetInputData(skullModel)
        ...
        cylinderTransformFilter = vtk.vtkTransformPolyDataFilter()
        cylinderTransformFilter.SetInputData(holeCylinder)
        cylinderTransformFilter.SetTransform(transformWithTranslate)
  ...
  def onPerformVesselSegmentation(self):
    ...
    if self.startVesselSegmentationButton.isChecked():
      self.deactivateOtherButtonsAndModel(currentButton=self.startVesselSegmentationButton)
  ...
  def onSaveData(self):
    if not self.isLoadingCase:
      ...
      nodeAttributes=["rel_ventricleVolume", "rel_model","rel_nasion", "rel_kocher", "rel_sagittalPoint","rel_target","rel_distal", "rel_vesselSeeds", \
                      ...
                      "rel_vesselnessWithMarginModel","rel_vesselnessVolume","rel_maskROI"]
      ...
      printModelAttributes = ["rel_printModel"]
  ...
  def onConnectedComponentCalc(self):
    # Exit if not at step yet
    targetNodeID = self.logic.baseVolumeNode.GetAttribute("vtkMRMLScalarVolumeNode.rel_target")
    distalNodeID = self.logic.baseVolumeNode.GetAttribute("vtkMRMLScalarVolumeNode.rel_distal")
    nasionNodeID = self.logic.baseVolumeNode.GetAttribute("vtkMRMLScalarVolumeNode.rel_nasion")
    if not (targetNodeID or distalNodeID or nasionNodeID):
      return
    targetNode = slicer.mrmlScene.GetNodeByID(targetNodeID)
    distalNode = slicer.mrmlScene.GetNodeByID(distalNodeID)
    nasionNode = slicer.mrmlScene.GetNodeByID(nasionNodeID)
    if not (targetNode.GetNumberOfFiducials() and distalNode.GetNumberOfFiducials() and nasionNode.GetNumberOfFiducials()):
      return
...
class VentriculostomyPlanningLogic(ScriptedLoadableModuleLogic, ModuleLogicMixin):
  ...
  def __init__(self):
    ...
    self.morphologyParameters = [[1,1,1], [4,4,4]]
    self.distanceMapThreshold = 100
    self.venousMargin = 5.0 #in mm
    self.minimalVentricleLen = 5.0 # in mm
    self.yawAngle = 0.0
    self.pitchAngle = 0.0
  ...
  def enableAuxilaryNodes(self):
    ...
    self.guideAttachmentNode = slicer.mrmlScene.CreateNodeByClass("vtkMRMLLabelMapVolumeNode")
    self.guideAttachmentNode.SetName("Attachment")
    slicer.mrmlScene.AddNode(self.guideAttachmentNode)    
  ...
  def clear(self):
    ...
    if self.guideVolumeNode:
      ...
      self.guideVolumeNode = None
    if self.guideAttachmentNode:
      slicer.mrmlScene.RemoveNode(self.guideAttachmentNode.GetDisplayNode())
      slicer.mrmlScene.RemoveNode(self.guideAttachmentNode)
      self.guideAttachmentNode = None      
  ...
  # connectedImage should be the image of the vessels, and it is in turn padded with the venousMargin
    ...
    if imageCollection:
      ...
      if marginNode and (marginNode.GetAttribute("vtkMRMLModelNode.modelCreated") == "False"):
        ...
        try:
          ...
          slicer.util.warningDisplay("distance map calucation failed, try use different settings")
          return None
  ...
  def makePathMeetAllConditions(self, path, approachablePoints, polyData, posKocher, posNasion, surfacePolyData):
    ...
    if approachablePoints != 0 and numOfRef >=1:
      for pointIndex in range(1, len(path), 2):
          ...
          #isPosteriorEnough = (abs(x[1] - posNasion[1]) > self.posteriorMargin)
  ...
  # Create vessel map using machine-learning via external call to NiftyNet and create connectedImage to make vessel model and for later use
  def calculateConnectedComponent(self,inputVolumeNode, vesselnessModelNode):
      if inputVolumeNode:
        dir_path = os.path.dirname(os.path.realpath(__file__))

        # Strip skull from image
        skullRemoval = slicer.modules.skullremoval
  ...
  def enableRelatedROI(self, attributeName, roiName, visibility = True):
    roiNode = None
    if self.baseVolumeNode:
      enabledAttributeID = self.baseVolumeNode.GetAttribute(attributeName)
      if enabledAttributeID and slicer.mrmlScene.GetNodeByID(enabledAttributeID):
        roiNode = slicer.mrmlScene.GetNodeByID(enabledAttributeID)
      else:
        ...
        self.baseVolumeNode.SetAttribute(attributeName, roiNode.GetID())
    return roiNode    
  ...
  def createEntryPoint(self) :
    ...
    maskROI_ID = self.baseVolumeNode.GetAttribute("vtkMRMLScalarVolumeNode.rel_maskROI")
    maskROI = slicer.mrmlScene.GetNodeByID(maskROI_ID)
    ...
    if inputModelNode and (inputModelNode.GetAttribute("vtkMRMLModelNode.modelCreated") == "True") and (nasionNode.GetNumberOfMarkups()) and sagittalReferenceLength and coronalReferenceLength:
      ...
      if polyData and self.trueSagittalPlane and nasionNode.GetNumberOfMarkups()> 0:
        ...
        # Create face mask ROI
        self.generateFaceMaskROI(maskROI)
  ...
  def generateFaceMaskROI(self, roiNode):
    if (roiNode.GetAttribute("ROINode.initialized") == "Initialized"):
      return roiNode
    else:
      ...
      roiNode.SetXYZ(midpointPos[0], midpointPos[1], midpointPos[2])
      roiNode.SetRadiusXYZ(80, 60, maskHeight/2)

      # Hacky way to signal that initialization is complete
      roiNode.SetAttribute("ROINode.initialized", "Initialized")
  ...
  def generateLabelMapForFaceMask(self):
    ...
    # TODO: Mask ROS currently doesn't respect sagittalYawAngle
    maskROI_ID = self.baseVolumeNode.GetAttribute("vtkMRMLScalarVolumeNode.rel_maskROI")
    maskROI = slicer.mrmlScene.GetNodeByID(maskROI_ID)
    baseDimension = [160,120,1000]
    maskROI.GetRadiusXYZ(baseDimension)
    baseDimension[0] = baseDimension[0]*2
    baseDimension[1] = baseDimension[1]*2
    baseDimension[2] = baseDimension[2]*2
  ...
  def getGuidanceCubeBoundary(self):
    #Create Guidance Cube boundaries for both vertical and horizontal directions
    holeDiameter = self.guideHoleDiameter
    posNasion = [0.0]*3
    ...
    centerPosH = [0.5*(posEntry[0]+posCoronal[0]), posEntry[1], 0.5*(posTop[2]+posNasion[2])]
    dimensionH_X = abs(posEntry[0]-posCoronal[0]) + holeDiameter + 10.0
    guidanceDimensionH = [dimensionH_X, holeDiameter + 10.0, abs(posTop[2]-posNasion[2])+20]
    
    return centerPosV, guidanceDimensionV, centerPosH, guidanceDimensionH
