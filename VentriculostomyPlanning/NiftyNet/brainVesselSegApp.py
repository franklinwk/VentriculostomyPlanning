...
class brainVesselSegApp(BaseApplication):
    REQUIRED_CONFIG_SECTION = "SEGMENTATION"

    def __init__(self, net_param, action_param, action):
        BaseApplication.__init__(self)
        tf.logging.info('starting brainVesselSegApp segmentation app')
        if action in [True, False]: # compatibility with NN 0.2
            # action parameter was previously is_training
            self.is_training = action
        else: 
            self.action = action
